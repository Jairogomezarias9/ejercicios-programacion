...
private:
  ...
  // Descomenteu les següents dues linies i implementeu el mètode:
   static int numTwinsAux(node_arbre *n){
      ...
      if (n->segE != NULL and n->segD != NULL)
     {
       if (n->segE->info == n->segD->info)
       {
          return 1;
       }
       return numTwinsAux(n->segE)+numTwinsAux(n->segD);
     }
     else if (n->segE != NULL and n->segD == NULL)
     {
        return numTwinsAux(n->segE);
     }
     else if (n->segE == NULL and n->segD != NULL)
     {
        return numTwinsAux(n->segD);
     }
     return 0;
