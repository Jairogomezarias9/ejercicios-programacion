...
int main()
{
    ...
    for (int i = 0; i < m; ++i)
    {
        ...
        taulell[v].push_back(make_pair(c,u));
    }
    int x;
    ...
    while (cin >> x >> y)
    {
        vector<int> pred;
        vector<double> d;
        vector<int> coste_aux = coste;
        coste_aux[x] = 0; // No cobramos por dormir en el origen
        coste_aux[y] = 0; // No cobramos por dormir en el destino
        distra(x, pred, d, taulell, coste_aux);
        if (d[y] != -1)
        {
            cout << "c(" << x << "," << y << ")" << " = " << d[y] << endl;
        }
