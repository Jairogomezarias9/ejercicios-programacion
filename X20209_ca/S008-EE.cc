...
public:
  ...
  class iterator {
    ...
    // Also, add the necessary adaptations so that, the move does not take place
    // when there is an active 'avoid' and such a move implies pointing to the same place as
       ...
       el resultat és el p.i. */
    {
      ...
      else if (nuevo->pitem != NULL and pitem->next!=NULL)
      {
         if (salta)
        {
          if (&(pitem->next) != &(nuevo->pitem))
          {
            pitem = pitem->next;
       ...
       el resultat és el p.i. */
    {
      ...
      else if (nuevo->pitem != NULL and pitem->prev!=NULL)
      {
         if (salta)
        {
          if (&(pitem->prev) != &(nuevo->pitem))
          {
            pitem = pitem->prev;
    ...
    // Pre: 'it' != 'this'
    // Post: Once executed, any move attempt (++ or --) on 'this' will cause no change
