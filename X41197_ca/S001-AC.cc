...
public:
  ...
  // Descomenteu les següents dues linies i implementeu el mètode:
   void moveToEnd(iterator &it) {
    if(it.pitem->next!=&itemsup){
      Item *elemento=it.pitem;
      Item *anterior=elemento->prev;
      Item *posterior=elemento->next;
      
      anterior->next=posterior;
      posterior->prev=anterior;
      elemento->next=&itemsup;
      elemento->prev=itemsup.prev;
      itemsup.prev->next=elemento;
      itemsup.prev=elemento;
